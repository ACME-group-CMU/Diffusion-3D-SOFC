...
class Microstructures(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.apply_sym:
            ...
            if np.random.choice([True, False]): subimage = torch.flip(subimage, dims=[0])
            if np.random.choice([True, False]): subimage = torch.flip(subimage, dims=[1])
            if np.random.choice([True, False]): subimage = torch.flip(subimage, dims=[2])
